 ...
 */
#include "native_src_bq_common_platform_thread_condition_variable_win64.h"
#ifdef BQ_WIN
            ...
            if (platform_data_) {
                if (initialize_cv_func_) {
                    ...
                    if (!platform_data_->handle_.condition_variable_handle_) {
                        bq::util::log_device_console(log_level::fatal, "%s : %" PRId32 " : create condition_variable failed, error code:%" PRId32, __FILE__, __LINE__, static_cast<int32_t>(GetLastError()));
