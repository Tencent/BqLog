...
func Test_concurrent_arguments(t *testing.T) {
			...
			for worker := 0; worker < workers; worker++ {
				...
				go func(worker int) {
					...
					for index := 0; index < count; index++ {
						...
						var written bool
						if index%2 == 0 {
							written = log.Info("record|{}|{}|{}|{}", worker, index, -int64(worker*count+index), payload)
						} else {
...
func Test_invalid_category_and_level(t *testing.T) {
...
func Test_go_stack_levels(t *testing.T) {
...
func Test_console_callback_count(t *testing.T) {
...
func Test_zero_argument(t *testing.T) {
	log := new_log(t, "sync", "snapshot.buffer_size=65536\nsnapshot.levels=[all]\n", nil)
...
func Test_argument_types_and_fallback(t *testing.T) {
	...
	cases := []struct {
		...
		{"null", []any{bq.Arg{}}, "null"},
		{"bool", []any{bq.Bool(false), bq.Bool(true)}, "FALSE|TRUE"},
		{"int8", []any{bq.I8(-128), bq.U8(255)}, "-128|255"},
		{"int16", []any{bq.I16(-32768), bq.U16(65535)}, "-32768|65535"},
		{"int32", []any{bq.I32(-2147483648), bq.U32(4294967295)}, "-2147483648|4294967295"},
		{"int64", []any{bq.I64(-9223372036854775808), bq.U64(^uint64(0))}, "-9223372036854775808|18446744073709551615"},
		{"floats", []any{bq.F32(1.25), bq.F64(-2.5), bq.Nil()}, "1.2500000|-2.500000000000000|null"},
		{"strings", []any{bq.Str(""), bq.Str("中文😀"), bq.Str(strings.Repeat("x", 5000)), bq.Int(42)}, "|中文😀|" + strings.Repeat("x", 5000) + "|42"},
	}
...
func Test_direct_write_callback_gc(t *testing.T) {
	bq.Set_console_buffer_enable(false)
	...
	for worker := 0; worker < 4; worker++ {
		...
		go func(worker int) {
			...
			for index := 0; index < 20; index++ {
				...
				if !log.Info("{} {} {}", str, index, strings.Clone(str)) {
					failed.Store(true)
				}
			}
		}(worker)
...
func Benchmark_concurrent_arguments(b *testing.B) {
...
func Test_snapshot_concurrency(t *testing.T) {
