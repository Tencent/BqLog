...
namespace bq {
    namespace test {
        ...
        constexpr auto char_params = test_make_array<char>('a', '1', '\t');
        constexpr auto char16_params = test_make_array<char16_t>(u'a', u'1', u'\t');
        constexpr auto char32_params = test_make_array<char32_t>(U'a', U'飞', (char32_t)(0x0001F600));
        constexpr auto int8_params = test_make_array<int8_t>((int8_t)1, (int8_t)-123);
        constexpr auto uint8_params = test_make_array<uint8_t>((uint8_t)1, (uint8_t)255);
        constexpr auto int16_params = test_make_array<int16_t>((int16_t)1, (int16_t)32532, (int16_t)-22123);
        constexpr auto uint16_params = test_make_array<uint16_t>((uint16_t)1, (uint16_t)65531);
        constexpr auto int32_params = test_make_array<int32_t>((int32_t)1, (int32_t)-22123, (int32_t)0xffff3223);
        constexpr auto uint32_params = test_make_array<uint32_t>((uint32_t)1, (uint32_t)0x324242fa, (uint32_t)0xffffffff);
        constexpr auto int64_params = test_make_array<int64_t>((int64_t)20, (int64_t)-5, (int64_t)0x32452343ffffffff, (int64_t)0xffaaeeff33333333);
        ...
        constexpr auto utf8_params = test_make_array<const char*>(nullptr, "a", "ab", "utf8测试代码看看呢");
        constexpr auto utf16_params = test_make_array<const char16_t*>(nullptr, u"a", u"ab", u"utf16测试代码看一看");
        constexpr auto utf32_params = test_make_array<const char32_t*>(nullptr, U"a", U"abc", U"utf16测试代码看一看");
        constexpr auto wchar_array_params = test_make_array<const wchar_t*>(nullptr, L"a", L"ab", L"utf16测试代码看一看");
                ...
                *output_str_ptr = output_str_ptr->substr(log_head.size(), output_str_ptr->size() - log_head.size());
                ...
                if (current_tested_num % snapshot_idx_mode == 0) {
                    ...
                    if (!snapshot.is_empty()) {
                        result_ptr->add_result(snapshot_test_str.end_with(snapshot) || snapshot.end_with(snapshot_test_str), "snapshot test failed, index:%zu, standard: %s,  \nsnapshot: %s", current_tested_num, output_str_ptr->c_str(), snapshot.substr(snapshot.size() > output_str_ptr->size() ? snapshot.size() - output_str_ptr->size() : 0, snapshot.size() > output_str_ptr->size() ? output_str_ptr->size() : snapshot.size()).c_str());
                    }
