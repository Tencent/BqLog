...
function run_command(command, args) {
    ...
    // On macOS, DYLD_INSERT_LIBRARIES is stripped when spawning via shell (/bin/sh).
    // We use BQ_ASAN_LIB to propagate it if needed.
    if (process.platform === 'darwin' && process.env.BQ_ASAN_LIB) {
        full_command = `DYLD_INSERT_LIBRARIES="${process.env.BQ_ASAN_LIB}" ${full_command}`;
    }
...
try {
    ...
    const cjs_result = run_command('node', ['-r', 'ts-node/register', 'src/bq/test/test_main_cjs.ts']);
    if (cjs_result !== 0) {
    ...
    const esm_result = run_command('node', ['--loader', 'ts-node/esm', 'src/bq/test/test_main_esm.ts']);
    if (esm_result !== 0) {
