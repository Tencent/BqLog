...
namespace bq {
        ...
        while (true) {
            ...
            if (c == '{') {
                ...
                if (!align && !fill) {
                    ...
                    fill = true;
                } else if (isdigit((int32_t)(uint8_t)c)) {
                    if (width[0] == '0')
                        width[0] = c;
                    else if (width[1] == 0)
                        width[1] = c;
    ...
    void layout::insert_integral_unsigned(uint64_t value, int32_t base /* = 10 */)
    {
        ...
        revert(begin_cursor, format_content_cursor - 1);
        if (isE) {
    ...
    void layout::insert_integral_signed(int64_t value, int32_t base /* = 10 */)
    {
        assert(base <= 32 && base >= 0 && "base is a number belongs to [2, 32]");
        // Hex check
        if (format_info_.type == 'b') {
    ...
    {
        auto precision = (format_info_.precision != 0xFFFFFFFF) ? format_info_.precision : 7;
        if (value >= 0) {
            uint64_t i_part = static_cast<uint64_t>(value);
            insert_integral_unsigned(i_part, 10);
            value -= static_cast<float>(i_part);
        } else {
            int64_t i_part = static_cast<int64_t>(value);
            insert_integral_signed(i_part, 10);
            value -= static_cast<float>(i_part);
        }
        if (format_info_.type == 'e')
            return;
        value = fabsf(value);
        expand_format_content_buff_size(format_content_cursor + precision + 1);
        if (precision > 0) {
            format_content[format_content_cursor++] = '.';
        }
        while (precision > 0) {
    ...
    {
        auto precision = (format_info_.precision != 0xFFFFFFFF) ? format_info_.precision : 15;
        if (value >= 0) {
            uint64_t i_part = static_cast<uint64_t>(value);
            insert_integral_unsigned(i_part, 10);
    ...
    void layout::revert(uint32_t begin_cursor, uint32_t end_cursor)
    {
