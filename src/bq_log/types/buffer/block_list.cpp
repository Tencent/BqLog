 ...
 */
 ...
 namespace bq {
    ...
    void block_list::reset(uint16_t max_blocks_count, uint8_t* buffers_base_addr, size_t blocks_total_buffer_size)
    {
        ...
        assert((buffer_size_per_block_ > (size_t)block_node_head::get_buffer_data_offset()) && "too small of block buffer size");
    }

    bool block_list::try_recover_from_memory_map(uint16_t max_blocks_count, uint8_t* buffers_base_addr, size_t blocks_total_buffer_size)
    {
        ...
        size_t buffer_size_per_block = (size_t)(blocks_total_buffer_size / max_blocks_count);
    ...
    void block_list::construct_blocks()
    {
        ...
        while (current_node) {
            new ((void*)current_node, bq::enum_new_dummy::dummy) block_node_head(
                (uint8_t*)current_node + block_node_head::get_buffer_data_offset(), buffer_size_per_block_ - (size_t)block_node_head::get_buffer_data_offset(), true);
